    ...
    for i, s in enumerate(data.get("sources", [])):
        ...
        if telegram_data:
            ...
            if not tg_token or not tg_chat_id:
                logger.warning(f"Source at index {i} skipped: Missing 'token' or 'chat_id'.")
                continue
        ...
        if not all([source_id, source_type, include_formats is not None]):
            logger.warning(f"Source at index {i} skipped: Missing required fields: 'id', 'type', or 'selector.include_formats'.")
            continue
